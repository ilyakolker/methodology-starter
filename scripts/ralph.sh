...
for ((ITER=1; ITER<=MAX_ITER; ITER++)); do
  ...
  if [[ "$TASK_CATEGORY" != "ui" ]]; then
    ...
    UI_VERIFY_BLOCK='   - category:ui (verification_mode=visual-review) — Use the Playwright runner: `npx playwright test e2e/<spec>.spec.ts --grep "<test-name>" --reporter=line`. Tests use the shared fixtures in `e2e/fixtures.ts` (per-test UUID isolation via a testTag fixture, trustedPage with auth cookie pre-set, domain-specific seed helpers) — NEVER write standalone .mjs scripts and NEVER full-table DELETE. The spec MUST capture screenshots via `await page.screenshot({ path: "screenshots/<task-id>/<viewport>.png" })` AND run DOM assertions. After the Playwright command exits 0, you MUST Read every screenshot via the Read tool and quote ONE specific visual property per screenshot in your report. NEGATIVE checks ("verify X is ABSENT") are first-class. Each test() block should be short (≤ ~25 lines); one scenario cluster per spec file (1-4 test() blocks max).'
  else
    # dom-only is the default for UI tasks when verification_mode is missing or set to "dom-only".
    UI_VERIFY_BLOCK='   - category:ui (verification_mode=dom-only) — Use the Playwright runner: `npx playwright test e2e/<spec>.spec.ts --grep "<test-name>" --reporter=line`. Tests use the shared fixtures in `e2e/fixtures.ts` (per-test UUID isolation via a testTag fixture, trustedPage with auth cookie pre-set, domain-specific seed helpers) — NEVER write standalone .mjs scripts and NEVER full-table DELETE; cleanup is automatic via testTag-prefixed row filtering. The spec MUST run DOM assertions (text content, attributes, computed styles, counts, focus, navigation, ARIA, getBoundingClientRect, presence/absence). Capture screenshots to screenshots/<task-id>/<viewport>.png for on-disk human review. You DO NOT Read the screenshots back via the Read tool — that step is intentionally skipped to save vision tokens. The gate is: Playwright exits 0 AND every DOM assertion fires. NEGATIVE checks ("verify X is ABSENT") are first-class. Each test() block should be short (≤ ~25 lines); one scenario cluster per spec file (1-4 test() blocks max).'
  fi
